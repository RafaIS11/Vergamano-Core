...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'arena' | 'lsd' | 'map' | 'enlace' | 'mercado'>('arena');
  ...
  return (
    <div className={`min-h-screen bg-white text-black font-mono p-0 overflow-x-hidden ${isLowHP ? 'filter grayscale contrast-150' : ''}`}>
        ...
        <div className="text-right flex flex-col items-end gap-2">
          <div className="flex gap-4">
            <div className="text-right border-r-4 border-black pr-4">
              ...
              <p className={`text-4xl font-black ${isLowHP ? 'text-red-600 animate-pulse' : 'text-black'}`}>
                {isLowHP ? 'CRÍTICA' : 'ESTABLE'}
              </p>
            </div>
        ...
        <section className="col-span-12 lg:col-span-9 p-12 border-r-[10px] border-black">
          ...
          {activeTab === 'enlace' && (
            <div className="space-y-4">
              <h2 className="text-6xl font-black italic border-b-8 border-black pb-4">ENLACE_NEURONAL</h2>
              <div className="p-12 border-8 border-black bg-gray-100">
                <p className="text-2xl font-bold">MOLTBOT ESTÁ LISTO PARA TU PRÓXIMA ACCIÓN.</p>
              </div>
            </div>
          )}
          {activeTab === 'map' && (
          ...
          {activeTab === 'mercado' && (
            <div className="text-center p-20 border-8 border-dashed border-black">
              <h2 className="text-6xl font-black">[MERCADO_CERRADO]</h2>
              <p className="text-xl font-bold mt-4">EL CONSEJO DE ADMINISTRACIÓN AÚN NO HA HABILITADO LAS TRANSACCIONES.</p>
            </div>
          )}
        </section>
        ...
        <aside className="col-span-12 lg:col-span-3 p-8 bg-white space-y-8">
          ...
          <div className="border-8 border-black p-4 space-y-4">
            <div className="flex justify-between items-end border-b-2 border-black pb-2">
              ...
              <span className="text-3xl font-black">{profile?.credits || 0} CR</span>
            </div>
